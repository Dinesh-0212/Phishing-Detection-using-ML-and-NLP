...
app = Flask(__name__)
app.secret_key = os.urandom(24)  # Generates a new secret key on each restart, clearing sessions
...
@app.route('/')
def home():
    return render_template('home.html', logged_in=session.get('logged_in', False), username=session.get('username', ''))
...
def login():
    if request.method == 'POST':
        ...
        # Simple authentication (Replace with database verification)
